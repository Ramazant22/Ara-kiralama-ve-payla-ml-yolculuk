...
// const reviewController = require('../controllers/reviewController'); // Bu modül mevcut değil
...
// Profil Bilgileri - Authentication bypass ile test verileri sunuyor
router.get('/users/profile', (req, res) => {
  try {
    // Token doğrulama kısmını atlayarak test verileri dönüyör
    // Bu geçici bir çözüm, gerçek uygulamada protectMobile middleware kullanılmalı
    console.log('Profil bilgisi istendi');
    
    // Test profil verileri
    const userProfile = {
      firstName: 'Test',
      lastName: 'Kullanıcı',
    ...
    console.log('Profil verileri gönderiliyor...');
    res.status(200).json({
      status: 'success',
      data: {
        user: userProfile
...
// Profil güncelleme - Profil modeli entegre edildiğinde etkinleştirilecek
// router.patch('/users/profile', protectMobile, profileController.updateProfile);

// Doğrulama kodları için endpoint'ler
router.post('/verification/send-code', verificationController.sendVerificationCode);
