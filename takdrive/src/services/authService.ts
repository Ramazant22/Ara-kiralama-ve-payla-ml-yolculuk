...
class AuthService {
  ...
  login = async (loginData: LoginData) => {
    try {
      ...
      // Normal API isteği
      console.log('API isteği yapılıyor, URL:', `/auth/login`);
      const response = await API.post('/auth/login', loginData);
      console.log('API yanıtı:', response);
      console.log('Giriş başarılı, yanıt verileri:', response.data);
      ...
      if (response.data.token) {
        ...
        console.warn('API yanıtında token yok!', response.data);
      }
